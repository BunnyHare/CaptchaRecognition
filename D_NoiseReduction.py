...
def find_nearby_black_pixel(image, x, y, width, height):  # 返回image中坐标为x,y像素周围8个像素的黑色像素数量
    ...
    if x == 0:  # 第一列
        if y == 0:  # 左上角
            ...
            sum = is_black(image, x, y - 1) + is_black(image, x + 1, y - 1) + is_black(image, x + 1, y) + is_black(
                image, x + 1, y + 1) + is_black(image, x, y + 1)
            return sum
    elif x == width - 1:  # 最后一列
        if y == 0:  # 右上角
            sum = is_black(image, x - 1, y) + is_black(image, x, y + 1) + is_black(image, x - 1, y + 1)
        ...
        if y == 0:  # 第一行
            sum = is_black(image, x - 1, y) + is_black(image, x - 1, y + 1) + is_black(image, x, y + 1) + is_black(
                image, x + 1, y + 1) + is_black(image, x + 1, y)
            return sum
